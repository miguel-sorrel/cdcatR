                       ...
                       .inorder = T) %dopar% {
                         try({
                           ...
                           for (jjselect in 1:MAXJ) {
                             ...
                             # update itemSelect
                             GDI <- switch(itemSelect, 
                                           GDI = cdcatR:::GDI.M(LC.prob = LC.prob, mlogPost_GDI), 
                                           GDIrel = GDI.M.rel(LC.prob = LC.prob, Q = Q, prior = mlogPost_GDI),
                                           JSD = JSD.DICO.M(LC.prob = LC.prob, mlogPost_GDI), 
                                           MPWKL = MPWKL.M(LC.prob = LC.prob, mlogPost_GDI), 
                                           PWKL = PWKL.M(LC.prob = LC.prob, mlogPost_GDI, point.est = sample(which(mlogPost_GDI == max(mlogPost_GDI)), 1)), 
                                           random = runif(J, 0, 1)) 
                             if(!is.null(itemExposurecontrol)) {
                               ...
                               } else if (itemExposurecontrol == "restricprogressive") {
                                 GDI <- restrproggresive.f(jjselect, MAXJ, b, maxr = 1, GDI, item.usage, jjcatGDI, Q, N) #  no maxr
                               }
                             }
                             names(GDI) <- 1:length(GDI)
